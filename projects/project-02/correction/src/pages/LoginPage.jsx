...
import { fakeLogin } from "../api/fakeMoviesApi";
...
export default function LoginPage() {
  ...
  const mutation = useMutation({ mutationFn: fakeLogin, onSuccess: ({ token }) => { setToken(token); navigate("/"); } });

  return (
    <div className="login-container">
      <div className="login-card">
        <h1 className="login-title">🎬 Movie Hub</h1>
        <p className="login-subtitle">Sign in to access your watchlist</p>
        <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="login-form">
        ...
        <p style={{ textAlign: "center", color: "var(--text-secondary)", fontSize: "0.8rem", marginTop: "1rem" }}>Any email + password (min 4 chars)</p>
      </div>
    </div>
  );
}
